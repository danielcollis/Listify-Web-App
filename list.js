...
function updateWishlistName(name) {
    ...
    try {
        const docRef = doc(db, "wishlistSettings", "nameSettings");
        setDoc(docRef, { wishlistName: name }, { merge: true });
        console.log("Wishlist name saved to Firestore");
    } catch (error) {
        console.error("Error saving wishlist name to Firestore:", error);
    }
}
